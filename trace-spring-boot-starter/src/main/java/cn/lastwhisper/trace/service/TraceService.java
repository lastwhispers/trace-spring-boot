...
public class TraceService {
    ...
    private static Map<String, String> traceIdUrlMap = new ConcurrentHashMap<>();//防止rehash时链表循环引用
    // randomId,MethodNode(某个method)
    private static Map<String, MethodNode> randomIdMap = new ConcurrentHashMap<>();
    // traceId,MethodNode(完整实时调用链)
    private static Map<String, MethodNode> traceIdChainMap = new ConcurrentHashMap<>();
     ...
     * 获取controller的url、http method
     */
    public MethodNode getCallChain(String traceId) {
        return traceIdChainMap.get(traceId);
    }

    public MethodNode getMethodNode(String randomId) {
     ...
     *
     */
    public void saveGeneral(MethodNode methodNode) {
        String randomId = methodNode.getRandomId();
        if (randomIdMap.containsKey(randomId)) {
            logger.info("saveGeneral randomId:{} duplicate", randomId);
        } else {
            logger.info("saveGeneral randomId:{} save success", randomId);
        }
